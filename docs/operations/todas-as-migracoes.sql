...
grant execute on function public.create_employee_for_delivery(text, text, uuid)
  to authenticated;
-- =============================================================================
-- Acrescentar ao balcão passa a guardar o email.
--
-- A migração 0016 criou o colaborador sempre com email nulo. Fazia sentido
-- para quem pesquisa por número: ninguém pede o email a quem está à espera do
-- kit. Mas há quem pesquise POR email — e nesse caso o email já está escrito,
-- na barra de pesquisa, e deitá-lo fora tinha uma consequência concreta:
-- acrescentava-se a pessoa, e a pesquisa seguinte pelo mesmo email não a
-- encontrava.
--
-- O email continua opcional (assim é desde a migração 0012). O que muda é
-- poder ser guardado quando existe.
--
-- A função antiga tem de ser REMOVIDA e não substituída: uma lista de
-- argumentos diferente cria uma sobrecarga em vez de substituir, e o PostgREST
-- recusa uma chamada por nome quando há duas candidatas.
-- =============================================================================

drop function if exists public.create_employee_for_delivery(text, text, uuid);
...
begin
  ...
  -- Vazio é uma escolha; mal escrito é um erro que ninguém ia notar depois de
  -- gravado. Mesma regra da importação e da página de Colaboradores.
...
comment on function public.create_employee_for_delivery(text, text, text, uuid) is
  'Cria um colaborador a partir do ecrã de distribuição. Qualquer conta ativa, '
  'porque é quem está ao balcão que encontra os casos em falta. Só cria: '
  'número repetido é recusado com DUPLICATE_EMPLOYEE_NUMBER. O email é '
  'opcional e serve a quem chegou aqui a pesquisar por email.';

revoke execute on function public.create_employee_for_delivery(text, text, text, uuid)
  from public, anon;
grant execute on function public.create_employee_for_delivery(text, text, text, uuid)
  to authenticated;
-- =============================================================================
